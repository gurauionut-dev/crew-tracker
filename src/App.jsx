...
// Bonusuri individuale per user (dacă lipsește, folosim DEFAULT_BONUSES)
const DEFAULT_BONUSES = {
  montaj: 37.5, demontaj: 37.5, operare: 250,
  condus: 50, deplasare: 0, asistenta: 0,
};

// Acțiuni vizibile per user (dacă lipsește, toate sunt vizibile mai puțin "condus")
const USER_OVERRIDES = {
  ionut: {
    bonuses: { ...DEFAULT_BONUSES },          // same as default
    visibleActions: ["montaj","demontaj","operare","condus","deplasare","asistenta"],
  },
  daniel: {
    bonuses: { ...DEFAULT_BONUSES, montaj: 50, demontaj: 50 },
    visibleActions: ["montaj","demontaj","operare","deplasare","asistenta"],
  },
  stefan: {
...
function getUserBonuses(uid) {
  return USER_OVERRIDES[uid]?.bonuses || DEFAULT_BONUSES;
}
function getUserActions(uid) {
  const keys = USER_OVERRIDES[uid]?.visibleActions || ["montaj","demontaj","operare","deplasare","asistenta"];
...
function parseGCalEvents(items) {
  return (items || [])
    ...
    .map(ev => {
      ...
      else if (tl.includes("transport") || tl.includes("condus")) type = "condus";
      else if (tl.includes("deplasare"))                    type = "deplasare";
      return {
        id:       ev.id,
        title,
        start:    hasTime ? startD.toLocaleTimeString("ro-RO", { hour: "2-digit", minute: "2-digit" }) : "",
        end:      hasTime ? endD.toLocaleTimeString("ro-RO",   { hour: "2-digit", minute: "2-digit" }) : "",
    ...
    .reduce((acc, ev) => {
      if (!acc[ev.dayKey]) acc[ev.dayKey] = [];
...
export default function App() {
  ...
  // amount: custom override > user-specific bonus > default
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.45)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
      <div style={{ background:"#fff", borderRadius:16, padding:28, maxWidth:480, width:"100%", boxShadow:"0 20px 60px rgba(0,0,0,0.2)" }}>
        ...
        <div style={{ fontSize:13, color:"#666", marginBottom:20, lineHeight:1.7 }}>
          Pași pentru a obține un API Key gratuit:<br/>
          1. Mergi la <a href="https://console.cloud.google.com" target="_blank" rel="noreferrer" style={{ color:"#185FA5" }}>console.cloud.google.com</a><br/>
          2. Creează un proiect nou<br/>
          3. <strong>APIs & Services → Enable APIs</strong> → activează <strong>Google Calendar API</strong><br/>
          4. <strong>Credentials → Create Credentials → API Key</strong><br/>
          5. Restricționează la <strong>Calendar API</strong> și paste-uiește mai jos:
        </div>
        <input type="text" value={val} onChange={e => setVal(e.target.value)} placeholder="AIzaSy..." style={{ width:"100%", padding:"10px 14px", borderRadius:10, border:"1px solid #e0e0de", fontSize:14, color:"#1a1a18", marginBottom:16, outline:"none" }}/>
...
function LoginScreen({ onLogin }) {
  const crew    = TEAM.filter(m => !m.isViewer);
  const viewers = TEAM.filter(m => m.isViewer);
  return (
    <div style={{ minHeight:"100vh", background:"#f0f0ef", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:24, padding:20 }}>
      <div style={{ textAlign:"center" }}>
        ...
        <h1 style={{ fontSize:24, fontWeight:700, color:"#1a1a18", margin:"0 0 6px", letterSpacing:-0.5 }}>Crew Tracker</h1>
        <p style={{ fontSize:14, color:"#888", margin:0 }}>Selectează-ți profilul</p>
      </div>
      <div style={{ width:"100%", maxWidth:440 }}>
        <div style={{ fontSize:11, fontWeight:600, color:"#aaa", letterSpacing:"0.06em", textTransform:"uppercase", marginBottom:10 }}>Echipa</div>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:20 }}>
          {crew.map(m => (
            <button key={m.id} onClick={() => onLogin(m)} style={{ background:"#fff", border:"1px solid #e8e8e6", borderRadius:14, padding:"16px 12px", cursor:"pointer", display:"flex", flexDirection:"column", alignItems:"center", gap:8 }}
              ...
              <div style={{ width:44, height:44, borderRadius:"50%", background:m.bg, color:m.color, display:"flex", alignItems:"center", justifyContent:"center", fontSize:15, fontWeight:700 }}>{m.initials}</div>
              <div style={{ textAlign:"center" }}>
                <div style={{ fontSize:13, fontWeight:600, color:"#1a1a18" }}>{m.name}</div>
                <div style={{ fontSize:11, color:"#888", marginTop:1 }}>{m.role}</div>
              </div>
              {m.isChief && <span style={{ fontSize:10, background:"#E6F1FB", color:"#0C447C", padding:"1px 8px", borderRadius:20, fontWeight:600 }}>CHIEF</span>}
            </button>
          ))}
        </div>
        <div style={{ fontSize:11, fontWeight:600, color:"#aaa", letterSpacing:"0.06em", textTransform:"uppercase", marginBottom:10 }}>Contabilitate</div>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
          {viewers.map(m => (
            <button key={m.id} onClick={() => onLogin(m)} style={{ background:"#fff", border:"1px solid #e8e8e6", borderRadius:14, padding:"14px 12px", cursor:"pointer", display:"flex", flexDirection:"column", alignItems:"center", gap:8 }}
              onMouseEnter={e => { e.currentTarget.style.borderColor="#378ADD"; e.currentTarget.style.boxShadow="0 4px 16px rgba(55,138,221,0.15)"; }}
              onMouseLeave={e => { e.currentTarget.style.borderColor="#e8e8e6"; e.currentTarget.style.boxShadow="none"; }}>
              <div style={{ width:40, height:40, borderRadius:"50%", background:m.bg, color:m.color, display:"flex", alignItems:"center", justifyContent:"center", fontSize:14, fontWeight:700 }}>{m.initials}</div>
              <div style={{ textAlign:"center" }}>
                <div style={{ fontSize:13, fontWeight:600, color:"#1a1a18" }}>{m.name}</div>
                <div style={{ fontSize:11, color:"#888", marginTop:1 }}>Raport lunar</div>
              </div>
            </button>
          ))}
        </div>
  ...
  return (
    <div style={{ display:"grid", gridTemplateColumns:"1fr 320px", minHeight:"calc(100vh - 52px)" }}>
      <div style={{ padding:20, borderRight:"1px solid #e8e8e6", overflowY:"auto" }}>
        ...
        {calError && <div style={{ background:"#FCEBEB", border:"1px solid #F5C2C2", borderRadius:10, padding:"10px 14px", marginBottom:14, fontSize:13, color:"#A32D2D" }}>⚠️ Eroare Calendar: {calError}</div>}
            ...
            return (
              <div key={ev.id} onClick={() => setSelEvent(active?null:ev.id)}
                ...
                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6, gap:8 }}>
                  ...
                  <span style={{ fontSize:12, color:"#888", whiteSpace:"nowrap" }}>{ev.start}{ev.end?`–${ev.end}`:""}</span>
                </div>
                <div style={{ display:"flex", alignItems:"center", gap:6, flexWrap:"wrap" }}>
                  <Badge type={ev.type}/>
                  {ev.location && <span style={{ fontSize:12, color:"#888" }}>📍 {ev.location}</span>}
                  {appr==="approved" && <span style={{ marginLeft:"auto", fontSize:11, background:"#E1F5EE", color:"#085041", padding:"1px 8px", borderRadius:20, fontWeight:500 }}>✓ aprobat · +{fmtRON(bonus)}</span>}
      ...
      <div style={{ padding:20, background:"#fff", overflowY:"auto" }}>
        {!selEv ? (
          <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", height:"100%", gap:12, color:"#bbb" }}>
            ...
            <span style={{ fontSize:12, textAlign:"center", maxWidth:180, lineHeight:1.5 }}>Bifează acțiunile efectuate</span>
          </div>
        ) : (
          <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
            <div>
              <div style={{ fontSize:15, fontWeight:600, color:"#1a1a18", marginBottom:3 }}>{selEv.title}</div>
              <div style={{ fontSize:12, color:"#888" }}>{selEv.start}{selEv.end?`–${selEv.end}`:""}{selEv.location?` · ${selEv.location}`:""}</div>
            </div>
            {approval && (
                ...
                return (
                  <div key={ev.id} style={{ background:"#fff", border:"1px solid #e8e8e6", borderRadius:12, overflow:"hidden" }}>
                    <div style={{ padding:"11px 14px", display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
                      ...
                      <span style={{ fontSize:12, color:"#aaa" }}>{ev.start}{ev.end?`–${ev.end}`:""}</span>
          ...
          return (
            <div key={member.id} style={{ background:"#fff", border:"1px solid #e8e8e6", borderRadius:14, padding:16 }}>
              <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:12 }}>
                ...
                <div>
                  <div style={{ fontSize:14, fontWeight:600, color:"#1a1a18" }}>{member.name}</div>
                  <div style={{ fontSize:11, color:"#888" }}>{details.length} activări aprobate</div>
                </div>
              </div>
            ...
            <table style={{ width:"100%", borderCollapse:"collapse" }}>
              ...
              <tbody>
                {details.map(({ ev, acts, total:evT }, i) => (
                  <tr key={ev.id} style={{ borderBottom:"1px solid #f0f0ef", background:i%2===0?"#fff":"#fafaf9" }}>
                    <td style={S.td}>
                      ...
                      {ev.start && <div style={{ fontSize:11, color:"#aaa" }}>{ev.start}{ev.end?`–${ev.end}`:""}</div>}
                    </td>
              ...
              <tfoot>
                <tr style={{ background:"#E1F5EE" }}>
                  <td colSpan={4} style={{ padding:"10px 20px", fontSize:13, fontWeight:600, color:"#085041" }}>
                    TOTAL {fmtMonth(reportMonth).toUpperCase()}
                  </td>
...
function SettingsView({ user, showToast, apiKey, setApiKey, setShowApiSetup }) {
  return (
    <div style={{ padding:20, maxWidth:520, margin:"0 auto" }}>

      {/* Bonusuri individuale read-only */}
      <div style={{ background:"#fff", border:"1px solid #e8e8e6", borderRadius:14, padding:20, marginBottom:16 }}>
        <div style={{ fontSize:15, fontWeight:700, color:"#1a1a18", marginBottom:4 }}>Bonusuri per persoană</div>
        <div style={{ fontSize:13, color:"#888", marginBottom:16 }}>Configurate de Crew Chief. Pentru modificări contactați-l pe Ionuț.</div>
      ...
      <div style={{ background:"#E6F1FB", border:"1px solid #C2D9F0", borderRadius:14, padding:18 }}>
        ...
        <div style={{ fontSize:12, color:"#185FA5", marginBottom:12 }}>
          Calendar ID: <code style={{ background:"#fff", padding:"2px 6px", borderRadius:4, fontSize:10, wordBreak:"break-all" }}>{CALENDAR_ID}</code>
        </div>
        {apiKey ? (
